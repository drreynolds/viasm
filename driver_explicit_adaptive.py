...
plt.xlabel('$u_1$')
plt.ylabel('$u_2$')
plt.title('Orbit (Bogacki-Shampine, %i steps)' % (BS.get_num_steps()))
plt.savefig('adaptive_BS_orbit.png')
plt.figure()
plt.plot(Y_DP[:,0],Y_DP[:,2])
plt.xlabel('$u_1$')
plt.ylabel('$u_2$')
plt.title('Orbit (Dormand-Prince %i steps)' % (DP.get_num_steps()))
