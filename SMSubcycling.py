...
class SMSubcycling:
    ...
    def __init__(self, fs, B, FastSolver, H=0.0):
        ...
        if ((np.size(self.b,0) != self.s) or (np.size(self.A,0) != self.s) or
            ...
            raise ValueError("SMSubcycling ERROR: incompatible Butcher table supplied")
    ...
    def Evolve(self, tspan, y0, H=0.0, args=()):
        ...
        if (self.H == 0.0):
            raise ValueError("ERROR: SMSubcycling::Evolve called without specifying a nonzero step size")
        ...
        for iout in range(1,tspan.size):
            ...
            for n in range(N):
                ...
                if (not success):
                    print("SMSubcycling error in time step at t =", t)
                    return Y, False
